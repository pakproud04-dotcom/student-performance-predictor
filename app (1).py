...
with photo_col:
    # Looks for a photo file in the same folder as this app.
    # Upload your photo to GitHub with one of these exact names.
    photo_candidates = ["founder.jpg", "founder.jpeg", "founder.png"]
    photo_found = next((p for p in photo_candidates if os.path.exists(p)), None)
    if photo_found:
        st.image(photo_found, width=140)
